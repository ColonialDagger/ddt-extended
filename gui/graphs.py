...
class HealthGraphCanvas(FigureCanvas):
    def __init__(self, parent=None, time_window=60.0):
        """
        time_window: seconds of history to display
        """
        # --- Figure background ---
...
class DpsGraphCanvas(FigureCanvas):
    def __init__(self, parent=None, time_window=60.0):
        """
        Duplicate of HealthGraphCanvas, but plotting d(health)/dt.
        """
        # --- Figure background ---
        self.fig = Figure(figsize=(5, 3), facecolor="#1e1e1e")
        super().__init__(self.fig)
        ...
        self.ax.grid(axis="x", color="#FFFFFF", alpha=0.15, linewidth=1.0, zorder=5)
